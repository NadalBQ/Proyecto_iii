...
rs_m9_log <- function(step, detail = NULL) {
  prefix <- sprintf("[%s] [M9] %s", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), step)
  if (!is.null(detail) && nzchar(detail)) {
    message(sprintf("%s | %s", prefix, detail))
  } else {
    message(prefix)
  }
}

rs_m9_format_object_size <- function(x) {
...
rs_m9_match_ready_accidents <- function(matching_ready_sf, edges_sf, search_radius_m = rs_m9_search_radius_m, batch_size = rs_m9_batch_size_default) {
  ...
  rs_m9_log(
    "Matching-ready accidents received",
    sprintf(
      "rows=%s | batch_size=%s | total_batches=%s | edge_attrs_rows=%s | edge_attrs_size=%s",
      total_rows,
      batch_size,
      total_batches,
      nrow(edge_attrs),
      rs_m9_format_object_size(edge_attrs)
    )
  )
  ...
  for (batch_number in seq_along(batch_starts)) {
    batch_start <- batch_starts[[batch_number]]
    ...
    rs_m9_log(
      "Batch candidate search start",
      sprintf(
        "batch=%s/%s | rows=%s-%s | batch_n=%s",
        batch_number,
        total_batches,
        batch_start,
        batch_end,
    ...
    candidate_t0 <- Sys.time()
    candidate_list <- sf::st_is_within_distance(batch_sf, edges_sf, dist = search_radius_m)
    candidate_elapsed <- round(as.numeric(difftime(Sys.time(), candidate_t0, units = "secs")), 2)
    candidate_counts <- lengths(candidate_list)

    rs_m9_log(
      "Batch candidate search done",
      sprintf(
        "batch=%s/%s | elapsed_s=%.2f | candidate_count_p50=%s | candidate_count_p95=%s | candidate_count_max=%s",
        batch_number,
        total_batches,
        candidate_elapsed,
        stats::median(candidate_counts),
        as.numeric(stats::quantile(candidate_counts, 0.95)),
    ...
    batch_elapsed <- round(as.numeric(difftime(Sys.time(), batch_t0, units = "secs")), 2)
    rs_m9_log(
      "Batch fully processed",
      sprintf(
        "batch=%s/%s | elapsed_s=%.2f | matched_so_far=%s",
        batch_number,
        total_batches,
        batch_elapsed,
        sum(!vapply(results[seq_len(batch_end)], is.null, logical(1)))
      )
    )
  }

  rs_m9_log("Binding batch results", sprintf("list_length=%s", length(results)))
...
rs_m9_write_outputs <- function(matches_df, unmatched_df, quality_summary, thresholds, validation_summary, matching_note, output_paths) {
  rs_m9_log(
    "Writing CSV outputs",
    sprintf(
      "matches=%s | unmatched=%s | quality_rows=%s | validation_rows=%s",
      nrow(matches_df),
      nrow(unmatched_df),
  ...
  rs_m9_log("Preparing GeoJSON output", sprintf("matches_n=%s", nrow(matches_df)))
  ...
  rs_m9_log("GeoJSON written", output_paths$matches_geojson)
}
...
rs_run_m9_accidente_edge_matching <- function(accident_master, m8_result, paths, force_refresh = FALSE, batch_size = rs_m9_batch_size_default) {
  rs_m9_log(
    "Run started",
    sprintf(
      "accident_master_n=%s | force_refresh=%s | batch_size=%s",
  ...
  if (!force_refresh && rs_m9_cache_is_current(output_paths)) {
    rs_m9_log("Current cache detected", "Returning cached M9 artifacts.")
    cached <- rs_m9_read_cached_outputs(output_paths)
    cached$used_cache <- TRUE
    return(cached)
  }

  rs_m9_log("Loading canonical edges", m8_result$edges_path)
  edges_sf <- rs_m9_load_edges(m8_result)
  rs_m9_log(
    "Canonical edges loaded",
    sprintf("rows=%s | object_size=%s", nrow(edges_sf), rs_m9_format_object_size(edges_sf))
  )

  rs_m9_log("Preparing accidents for matching", sprintf("search_radius_m=%s", rs_m9_search_radius_m))
  ...
  rs_m9_log(
    "Accidents prepared",
    sprintf(
      "valid_coordinate_n=%s | inside_envelope_n=%s | outside_envelope_n=%s | invalid_coordinate_n=%s",
      prepared$valid_coordinate_n,
      prepared$inside_envelope_n,
      prepared$outside_envelope_n,
      nrow(prepared$invalid_unmatched)
    )
  )

  rs_m9_log("Running spatial matching", "This is the dominant heavy step in M9.")
  ...
  rs_m9_log("Spatial matching finished", sprintf("rows=%s", nrow(matching_results)))
  ...
  if (nrow(unmatched_valid_df)) {
    rs_m9_log(
      "Attaching nearest edge for unmatched diagnostics",
      sprintf("rows=%s", nrow(unmatched_valid_df))
    )
  ...
  rs_m9_log(
    "Built output tables",
    sprintf("matched=%s | unmatched=%s", nrow(matched_df), nrow(unmatched_df))
  )
  rs_m9_write_outputs(
    matches_df = matched_df,
    unmatched_df = unmatched_df,
  ...
  rs_m9_log("Run completed", "M9 outputs written successfully.")
